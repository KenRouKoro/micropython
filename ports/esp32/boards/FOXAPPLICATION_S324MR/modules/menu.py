...
class Menu:
    def __init__(self, bsp):
        ...
        self.window_start = 0
        self.visible_lines = (
            3  # 128x32 display, maybe 3-4 lines of text. 32px / 8px/line = 4 lines.
        )
    ...
    def show_progress_dialog(self, title, value, max_value=100):
        ...
        self.oled.fill_rect(
            bar_x + 2, bar_y + 2, int((bar_w - 4) * percent), bar_h - 4, 1
        )  # Fill with padding
    ...
    async def handle_input(self):
        ...
        key_pressed = False
        if (
            self.bsp.key_up.value() == 0
            or self.bsp.key_down.value() == 0
            or self.bsp.key_ok.value() == 0
            or self.bsp.key_cancel.value() == 0
        ):
            key_pressed = True
            ...
            if self.key_press_start_time == 0:
                ...
                if duration > 1000:  # After 1 second
                    self.acceleration_factor = min(
                        4, 1 + (duration - 1000) // 500
                    )  # Increase factor every 500ms, max 4
        else:
            self.key_press_start_time = 0
            self.acceleration_factor = 1
